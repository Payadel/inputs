...
export const getInputs = (defaultInputs: IInputs): IInputs => {
  return {
    ...
    verbose:
      getBooleanInputOrDefault('verbose', undefined) ?? defaultInputs.verbose
  }
}
...
function getValidatedYamlInput(defaultYamlInput: IYamlInput[]): IYamlInput[] {
  ...
  return parseYaml(yamlInputsStr)
}

function parseYaml(yamlInputsStr: string): IYamlInput[] {
  const parsedYaml = yaml.load(yamlInputsStr) as IYamlInput[]
  ...
  const repetitiveKeys = findRepetitiveItems(parsedYaml.map(item => item.name))
